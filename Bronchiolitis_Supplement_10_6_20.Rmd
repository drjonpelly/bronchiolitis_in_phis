...
```{r generate invasive vent and icu numeric columns}
...
```{r remove_few_patients_with_missing_sex_data}
sex_cleaning_function <- function(bronchiolitis_table) {
  bronchiolitis_table %>%
    filter(gender_title != "Unknown") %>%
    mutate(gender_title = factor(gender_title, levels = c("Male", "Female")))
}
all_bronchiolitis_diagnoses <- sex_cleaning_function(all_bronchiolitis_diagnoses)
primary_bronchiolitis_diagnoses <- sex_cleaning_function(primary_bronchiolitis_diagnoses)
apr_drg_bronchiolitis <- sex_cleaning_function(apr_drg_bronchiolitis)
rm(sex_cleaning_function)
```

```{r remove_patients_with_negative_age}
age_cleaning_function <- function(bronchiolitis_table) {
  bronchiolitis_table %>%
    filter(admit_age_in_months >= 0)
}

all_bronchiolitis_diagnoses <- age_cleaning_function(all_bronchiolitis_diagnoses)
primary_bronchiolitis_diagnoses <- age_cleaning_function(primary_bronchiolitis_diagnoses)
apr_drg_bronchiolitis <- age_cleaning_function(apr_drg_bronchiolitis)
rm(age_cleaning_function)
```

```{r hospital_region_generation}
region_generation_function <- function(bronchiolitis_table) {
  bronchiolitis_table <- bronchiolitis_table %>%
    mutate(hospital_region = case_when(
      hospital_state == "ME" |
        hospital_state == "NH" |
        hospital_state == "VT" |
        hospital_state == "MA" |
      ...
      hospital_state == "ID" |
        ...
        hospital_state == "OR" |
        hospital_state == "CA" |
        hospital_state == "HI" ~ "west"
    )) %>%
    mutate(hospital_region = factor(hospital_region))

  return(bronchiolitis_table)
}

primary_bronchiolitis_diagnoses <- region_generation_function(primary_bronchiolitis_diagnoses)
all_bronchiolitis_diagnoses <- region_generation_function(all_bronchiolitis_diagnoses)
apr_drg_bronchiolitis <- region_generation_function(apr_drg_bronchiolitis)

cat(paste("\n\nHere's a summary of the primary bronchiolitis diagnosis hospital regions:\n"))
summary(primary_bronchiolitis_diagnoses$hospital_region)

cat(paste("\n\nHere's a summary of the all bronchiolitis diagnosis hospital regions:\n"))
summary(all_bronchiolitis_diagnoses$hospital_region)

cat(paste("\n\nHere's a summary of the apr-drg bronchiolitis diagnosis hospital regions:\n"))
summary(apr_drg_bronchiolitis$hospital_region)
```
...
demographics_primary_bronchiolitis_diagnoses <- demographics_table_function(primary_bronchiolitis_diagnoses, " Primary Bronchiolitis Diagnoses", primary_bronchiolitis_diagnoses$discharge_id)

demographics_primary_bronchiolitis_diagnoses_no_ccc <- demographics_table_function(primary_bronchiolitis_diagnoses_no_ccc, "Primary Bronchiolitis Diagnoses Without CCC", primary_bronchiolitis_diagnoses_no_ccc$discharge_id)

demographics_primary_bronchiolitis_diagnoses_yes_ccc <- demographics_table_function(primary_bronchiolitis_diagnoses_yes_ccc, "Primary Bronchiolitis Diagnoses With CCC", primary_bronchiolitis_diagnoses_yes_ccc$discharge_id)
...
**The first tab** displays a table with the characteristics of patients with diagnostic codes for viral bronchiolitis.

Patients in the “Primary Bronchiolitis Diagnoses” cohort have a primary admission diagnosis of viral bronchiolitis. Patients in the “All Bronchiolitis Diagnoses” cohort have an ICD9 or ICD10 code for viral bronchiolitis associated with admission as a primary or secondary diagnosis code. The cohort in the last column is identified using the All Patient Refined Diagnosis Group, version 32 (APR-DRG v32) for viral bronchiolitis using 3M software (3M, St. Paul, MN).

**The second tab** shows show the percentage of admissions within a given discharge year that had CCC.

**The third tab** shows the relative composition of CCCs over time. CCCs are shown as a proportion of the total number of CCCs because a given admission may have more than one CCC (e.g a patient may be admitted with both a respiratory and a renal CCC).

## Demographics Table {.tabset .tabset-fade}
...
### CCC Admissions Over Time

```{r ccc_admission_over_time_graph, results = "hold", out.width="33%"}
ccc_admission_graph_function <- function(bronchiolitis_table, graph_name) {
  bronchiolitis_table %>%
    group_by(discharge_year) %>%
    summarise(
      "complex chronic condition" = sum(complex_chronic_condition_flag == "Y") / n() * 100
    ) %>%
    ggplot(aes(factor(discharge_year), `complex chronic condition`)) +
    geom_col(fill = "gray80") +
    geom_text(aes(label = paste0(round(`complex chronic condition`, 1), "%")), position = position_stack(vjust = 0.5)) +
    xlab("Discharge Year") +
    ylab("% of Admissions with CCC") +
    theme_bw() +
    labs(title = graph_name) +
    theme(plot.title = element_text(hjust = 0.5), axis.text.x = element_text(angle = 45, hjust = 1))
}

ccc_admission_graph_function(primary_bronchiolitis_diagnoses, "Primary Bronchiolitis Diagnoses")
ccc_admission_graph_function(all_bronchiolitis_diagnoses, "All Bronchiolitis Diagnoses")
ccc_admission_graph_function(apr_drg_bronchiolitis, "APR-DRG v32 Bronchiolitis")

rm(ccc_admission_graph_function)
```

### CCC Proportions Over Time

```{r ccc_over_time_graph, results = "hold"}
ccc_over_time_graph_function <- function(bronchiolitis_table, graph_name) {
  bronchiolitis_table %>%
    group_by(discharge_year) %>%
    summarise(
      "cardiovascular condition" = sum(cardiovascular_flag == "Y"),
    ...
    pivot_longer(cols = c(ends_with("condition"), "malignancy", "technology dependent", "transplant recipient")) %>%
    ggplot(aes(factor(discharge_year), value)) +
    geom_col(aes(fill = name), position = "fill") +
    xlab("Discharge Year") +
    ylab("Proportion of CCC") +
...
ccc_over_time_graph_function(primary_bronchiolitis_diagnoses, "Primary Bronchiolitis Diagnoses")
ccc_over_time_graph_function(all_bronchiolitis_diagnoses, "All Bronchiolitis Diagnoses")
ccc_over_time_graph_function(apr_drg_bronchiolitis, "APR-DRG v32 Bronchiolitis")

rm(ccc_over_time_graph_function)
```
...
yearly_graph_function <- function(bronchiolitis_table, admission_graph_name, patients_graph_name, icu_admission_graph_name, ecmo_graph_name) {
  ...
  ecmo_admission_graph <- ggplot(data = hospital_vs_bronchiolitis_admissions, mapping = aes(x = discharge_year)) +
    ...
    scale_y_continuous(sec.axis = sec_axis(~ . / 5000 * 100, name = "% Bronchiolitis (Black Trend Line)")) +
    ylab("n") +
    scale_x_continuous(name = "Discharge Year", breaks = c(2010:2019)) +
...
**The first four tabs** display the number of annual admissions, unique patients, ICU admissions, and admissions requiring ECMO over time. These graphs display the number of encounters for the selected PHIS hospitals among all diagnoses (dark gray) compared to bronchiolitis (light gray). The bars represent the absolute number of admissions, and the line (right axis) shows the trend in the proportion of bronchiolitis diagnoses over time.

These graphs show that the proportion of patients with a primary admission diagnosis of bronchiolitis is relatively constant over time, but that the number of patients with any admission diagnosis of bronchiolitis is increasing over the past decade.

Bronchiolitis represents a gradually increasing proportion of ICU admissions over time across all cohorts. The proportion of patients with any bronchiolitis diagnosis receiving ECMO is increasing over time, though the proportion receiving ECMO is stable in the primary diagnosis and APR-DRG cohorts.

**The last tab** shows the proportion of admissions with bronchiolitis admitted to the ICU over time. The first three graphs show the bronchiolitis cohorts. The last graph shows all admissions to the PHIS hospitals among children under two years old (with *any* diagnosis, not just bronchiolitis) for reference.
...
**The first tab** shows the proportion of patients receiving invasive (dark gray) and non-invasive (light gray) ventilation over time. The proportion of patients receiving non-invasive ventilation has significantly increased across all cohorts. The proportion of patients receiving invasive mechanical ventilation has not changed significantly in any cohort.

**The second tab** shows the use of invasive (dark gray) and non-invasive (light gray) ventilation by hospital. There was no significant correlation in any cohort.

**The third tab** shows the number of patients with bronchiolitis admitted to the ICU receiving no ventilatory support (lightest gray), non-invasive ventilation (medium gray), and invasive mechanical ventilation (darkest gray).

**The fourth tab** shows the percentage of patients with bronchiolitis admitted to the ICU receiving no ventilatory support (lightest gray), non-invasive ventilation (medium gray), and invasive mechanical ventilation (darkest gray).
...
**The first tab** shows the hospital length of stay over time for patients with bronchiolitis and **the third tab** shows the ICU length of stay. 

**The second tab** and **the fourth tab** show the hospital length of stay and the ICU length of stay, respectively, for patients with bronchiolitis grouped by their maximum level of ventilator support: none (light gray), non-invasive (medium gray), or invasive (dark gray).

Note that the plots of ICU length of stay include only patients admitted to the ICU. This cohort has a longer hospital length of stay compared to the cohort not admitted to an ICU, which is why the median ICU length of stay for this cohort is approximately the same as the median hospital length of stay for the cohorts of all patients with bronchiolitis.

Overall hospital length of stay and ICU length of stay are decreasing for patients with oxygen support only and patients with non-invasive ventilatory support.  Length of stay has not significantly changed over time for patients who receive invasive ventilation.
...
These graphs show cost of care over time. **The first tab** and **the second tab** show the annual cost adjusted for the Bureau of Economic Analysis Gross Domestic Product (GDP) and CMS National Healthcare Expenditures per Capita (NHEC), respectively.

**The third tab** and **the fourth tab** show the per-admission cost adjusted for GDP and NHEC, respectively.

**The fifth tab** and **the sixth tab** show the per-admission cost adjusted for GDP and NHEC, respectively, grouped by the level of ventilatory support.

For all grouped graphs, costs are grouped by level of ventilatory support: no ventilatory support (lightest gray), non-invasive ventilatory support (light gray), invasive ventilatory support (dark gray), and both invasive and non-invasive ventilatory support (darkest gray).

All graphs adjust for Centers for Medicare & Medicaid Services (CMS) cost-of living index adjust for cost-of-living in each hospital’s zip code. 

The graphs adjusted for the Bureau of Economic Analysis Gross Domestic Product (GDP) account for non-specific inflation and are expressed in 2010 dollars. The graphs adjusted for CMS National Healthcare Expenditures per Capita costs account for healthcare-specific inflation and are expressed in 2010 dollars.

Adjusted costs have significantly decreased over time for patients receiving non-invasive ventilation, invasive ventilation or both during hospitalization.
...
yearly_graph_function <- function(bronchiolitis_table, inflation_table, graph_name, graph_notes) {
  ...
  yearly_plot <- ggplot(data = yearly_table, mapping = aes(x = as.factor(discharge_year), y = total_inflation_adjusted_cost, fill = factor(total_vent_support, levels = c("both", "invasive", "noninvasive", "none")))) +
    ...
    xlab("Discharge Year") +
    labs(title = graph_name, caption = graph_notes, subtitle = ifelse(temp_p > 0.05, paste("There was no significant change in the annual cost of care over time."), paste("The annual cost of care is changing by $", round(temp_los, 1), " million per year, p", ifelse(temp_p < 0.001, "<0.001", paste("=", round(temp_p, 3), sep = "")), " for trend.", sep = ""))) +
...
cost_per_patient_function <- function(bronchiolitis_table, adjustment_table, graph_name) {
  ...
  temp_plot <- temp_table %>%
    ...
    xlab("Discharge Year") +
    ylab("Admission Cost ($)") +
    labs(title = graph_name, subtitle = paste("Mean cost per admission is changing by ", round(temp_delta), " dollars per year, trend p ", ifelse(temp_p < 0.001, "<0.001", paste("=", round(temp_p, 3), sep = "")), ".", sep = "")) +
    theme_bw() +
    theme(plot.title = element_text(hjust = 0.5)) +
    theme(plot.subtitle = element_text(hjust = 0.5))
...
cost_by_vent_support_function <- function(bronchiolitis_table, adjustment_table, graph_name) {
  ...
  temp_plot <- temp_table %>%
    ...
    xlab("Discharge Year") +
    ylab("Admission Cost ($)") +
    labs(title = graph_name, caption = paste("Mean cost for none is changing by ", round(temp_none_delta), " dollars per year, trend p ", ifelse(temp_none_p < 0.001, "<0.001", temp_none_p), ".\n", "Mean cost for noninvasive is changing by ", round(temp_noninvasive_delta), " dollars per year, trend p ", ifelse(temp_noninvasive_p < 0.001, "<0.001", round(temp_noninvasive_p, 3)), ".\n", "Mean cost for invasive is changing by ", round(temp_invasive_delta), " dollars per year, trend p ", ifelse(temp_invasive_p < 0.001, "<0.001", paste("= ", round(temp_invasive_p, 3), sep = "")), ".\n", "Mean cost for both is changing by ", round(temp_both_delta), " dollars per year, trend p ", ifelse(temp_both_p < 0.001, "<0.001", paste("= ", round(temp_both_p, 3), sep = "")), ".", sep = "")) +
    theme_bw() +
    theme(plot.title = element_text(hjust = 0.5)) +
    theme(legend.position = "none")
...
### Gross Domestic Product Adjusted Cost Per Admission
...
### National Health Expenditures per Capita Adjusted Cost Per Admission
...
# Training sets (100% of sample)

all_bronchiolitis_diagnoses_training <- all_bronchiolitis_diagnoses

primary_bronchiolitis_diagnoses_training <- primary_bronchiolitis_diagnoses

apr_drg_bronchiolitis_diagnoses_training <- apr_drg_bronchiolitis
```
...
model_development_function <- function(bronchiolitis_training_table) {
  message("\n\nBeginning model generation function. Loading required packages.\n\n")
  require(splines)
  require(lme4)
  require(lubridate)
  require(glue)
  ...
  t1 <- now()
  message(glue("Now running on {deparse(substitute(x))}.\n\n"))
  message(glue("\n\nGenerating generalized linear model.\nYour start time is: {t1}\n\n"))

  temp_model <- glm(formula = invasive_vent_flag ~ discharge_year + primary_source_of_payment_simplified + gender_title + race_factor + ethnicity_title + splines::bs(admit_age_in_months) + cardiovascular_flag + gastrointestinal_flag + hematologic_or_immunologic_flag + malignancy_flag + metabolic_flag + neurologic_and_neuromuscular_flag + other_congenital_or_genetic_defect_flag + premature_and_neonatal_flag + renal_and_urologic_flag + respiratory_flag + technology_dependent_flag + tpn_flag + transplant_flag, data = bronchiolitis_training_table, family = binomial(link = "logit"))

  t2 <- now()
  message(glue("\n\nGeneralized linear model complete. Your run time was: {round(dseconds(t1 %--% t2),2)}\n\nPerforming stepwise selection on generalized linear model.\nYour start time is: {t2}\n"))
  ...
  t3 <- now()
  message(paste("\n\nStepwise selection complete. Your run time was:", round(dseconds(t2 %--% t3), 2)))
  message(paste("\n\nPerforming mixed-modeling with random effect for hospital region.\nYour start time is:", t3))

  temp_mixed_model <- glmer(
    formula = as.formula(paste(paste(stepwise_temp_model$call)[[2]], "+ (1|hospital_region)")),
    data = bronchiolitis_training_table,
    family = binomial(link = "logit")
  )

  t4 <- now()
  message(paste("\n\nMixed-modeling complete.\nYour run time was:", round(dseconds(t3 %--% t4), 2)))


  number_of_hospitals <- nrow(bronchiolitis_training_table %>%
    group_by(hospital_city) %>%
    summarize(n = n(), .groups = "drop_last"))

  message(paste("\n\nThere were", number_of_hospitals, "hospitals in the sample."))
  t5 <- now()
  message(paste("\n\nModel generation function complete.\nYour run time was:", round(dseconds(t1 %--% t5), 2)))

  return(list(stepwise_temp_model, temp_mixed_model))
}

invasive_models_all <- model_development_function(all_bronchiolitis_diagnoses_training)
invasive_models_primary <- model_development_function(primary_bronchiolitis_diagnoses_training)
invasive_models_apr_drg <- model_development_function(apr_drg_bronchiolitis_diagnoses_training)
rm(model_development_function)


noninvasive_model_development_function <- function(bronchiolitis_training_table) {
  message("\n\nBeginning noninvasive model generation function. Loading required packages.\n\n")
  require(splines)
  require(lme4)
  require(geepack)
  require(lubridate)
  ...
  temp_model <- glm(formula = noninvasive_vent_flag ~ discharge_year + primary_source_of_payment_simplified + gender_title + race_factor + ethnicity_title + splines::bs(admit_age_in_months) + cardiovascular_flag + gastrointestinal_flag + hematologic_or_immunologic_flag + malignancy_flag + metabolic_flag + neurologic_and_neuromuscular_flag + other_congenital_or_genetic_defect_flag + premature_and_neonatal_flag + renal_and_urologic_flag + respiratory_flag + technology_dependent_flag + tpn_flag + transplant_flag, data = bronchiolitis_training_table, family = binomial(link = "logit"))
...
noninvasive_models_all <- noninvasive_model_development_function(all_bronchiolitis_diagnoses_training)
noninvasive_models_primary <- noninvasive_model_development_function(primary_bronchiolitis_diagnoses_training)
noninvasive_models_apr_drg <- noninvasive_model_development_function(apr_drg_bronchiolitis_diagnoses_training)

rm(noninvasive_model_development_function)


icu_admission_model_development_function <- function(bronchiolitis_training_table) {
  message("\n\nBeginning icu admission model generation function. Loading required packages.\n\n")
  require(splines)
  require(lme4)
  require(lubridate)
  require(glue)
  ...
  temp_model <- glm(formula = icu_flag ~ discharge_year + primary_source_of_payment_simplified + gender_title + race_factor + ethnicity_title + splines::bs(admit_age_in_months) + cardiovascular_flag + gastrointestinal_flag + hematologic_or_immunologic_flag + malignancy_flag + metabolic_flag + neurologic_and_neuromuscular_flag + other_congenital_or_genetic_defect_flag + premature_and_neonatal_flag + renal_and_urologic_flag + respiratory_flag + technology_dependent_flag + tpn_flag + transplant_flag, data = bronchiolitis_training_table, family = binomial(link = "logit"))
  ...
  stepwise_temp_model <- step(temp_model, direction = "both", trace = 0, scope = list(lower = as.formula(icu_flag ~ discharge_year), upper = as.formula(icu_flag ~ .)))
...
icu_models_all <- icu_admission_model_development_function(all_bronchiolitis_diagnoses_training)
icu_models_primary <- icu_admission_model_development_function(primary_bronchiolitis_diagnoses_training)
icu_models_apr_drg <- icu_admission_model_development_function(apr_drg_bronchiolitis_diagnoses_training)

rm(icu_admission_model_development_function)

model_end_time <- now()
```

```{r evaluate_model_characteristics}
model_summary_function <- function(mv_model, mixed_model) {
  message("This function will summarize three related models on the same dataset.\n\n")
  require(kableExtra)

  mv_tibble <- as_tibble(summary(mv_model)$coefficients) %>%
    mutate(Variable = rownames(summary(mv_model)$coefficients)) %>%
    mutate(odds_ratio = exp(Estimate)) %>%
    mutate(low_conf_int_95 = Estimate - `Std. Error`) %>%
    mutate(hi_conf_int_95 = Estimate + `Std. Error`) %>%
    mutate(or_low_conf_int_95 = exp(low_conf_int_95)) %>%
    mutate(or_hi_conf_int_95 = exp(hi_conf_int_95)) %>%
    mutate(or_conf_int_95 = paste(round(odds_ratio, 3), " [", round(or_low_conf_int_95, 3), ", ", round(or_hi_conf_int_95, 3), "]", sep = "")) %>%
    select(Variable, "Fixed-Effects Odds Ratio [95% CI]" = or_conf_int_95, "Fixed-Effects P-Value" = `Pr(>|z|)`) %>%
    mutate(`Fixed-Effects P-Value` = round(`Fixed-Effects P-Value`, 3))

  mixed_tibble <- as_tibble(summary(mixed_model)$coefficients) %>%
    mutate(Variable = rownames(summary(mixed_model)$coefficients)) %>%
    mutate(odds_ratio = exp(Estimate)) %>%
    mutate(low_conf_int_95 = Estimate - `Std. Error`) %>%
    mutate(hi_conf_int_95 = Estimate + `Std. Error`) %>%
    mutate(or_low_conf_int_95 = exp(low_conf_int_95)) %>%
    mutate(or_hi_conf_int_95 = exp(hi_conf_int_95)) %>%
    mutate(or_conf_int_95 = paste(round(odds_ratio, 3), " [", round(or_low_conf_int_95, 3), ", ", round(or_hi_conf_int_95, 3), "]", sep = "")) %>%
    select(Variable, "Mixed-Effects Odds Ratio [95% CI]" = or_conf_int_95, "Mixed-Effects P-Value" = `Pr(>|z|)`) %>%
    mutate(`Mixed-Effects P-Value` = round(`Mixed-Effects P-Value`, 3))

  combined_tibble <- full_join(mv_tibble, mixed_tibble, by = "Variable") %>%
    filter(Variable != "(Intercept)") %>% # remove intercepts (meaningless)
    mutate("Variable" = case_when(
      ...
      Variable == "premature_and_neonatal_flagY" ~ "CCC: Prematurity / Neonatal",
      ...
      Variable == "transplant_flagY" ~ "CCC: Transplant Patient",
      Variable == "splines::bs(admit_age_in_months)1" ~ "Admit Age (months): 1st Order Spline",
      Variable == "splines::bs(admit_age_in_months)2" ~ "Admit Age (months): 2nd Order Spline",
      Variable == "splines::bs(admit_age_in_months)3" ~ "Admit Age (months): 3rd Order Spline",
      Variable == "gender_titleFemale" ~ "Sex: Female",
      TRUE ~ Variable
    ))
  return(knitr::kable(combined_tibble) %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), fixed_thead = F))
}

summary_all <- model_summary_function(invasive_models_all[[1]], invasive_models_all[[2]])
summary_primary <- model_summary_function(invasive_models_primary[[1]], invasive_models_primary[[2]])
summary_apr_drg <- model_summary_function(invasive_models_apr_drg[[1]], invasive_models_apr_drg[[2]])

noninvasive_summary_all <- model_summary_function(noninvasive_models_all[[1]], noninvasive_models_all[[2]])
noninvasive_summary_primary <- model_summary_function(noninvasive_models_primary[[1]], noninvasive_models_primary[[2]])
noninvasive_summary_apr_drg <- model_summary_function(noninvasive_models_apr_drg[[1]], noninvasive_models_apr_drg[[2]])

icu_summary_all <- model_summary_function(icu_models_all[[1]], icu_models_all[[2]])
icu_summary_primary <- model_summary_function(icu_models_primary[[1]], icu_models_primary[[2]])
icu_summary_apr_drg <- model_summary_function(icu_models_apr_drg[[1]], icu_models_apr_drg[[2]])
...
Results of multivariable logistic regression models with an outcome of invasive mechanical ventilation are presented. Models were developed by including race/ethnicity, age, sex, insurance status, complex chronic condition flags and discharge year. Automated backward stepwise selection was performed to minimize Akaike Information Criteria. Discharge year was forced into the model as it was a variable of interest. The mixed effects models contain random intercepts for hospital region.

**The first tab** shows the results for patients with a primary encounter diagnosis of bronchiolitis.

**The second tab** shows results for patients with any encounter diagnosis of bronchiolitis (primary or secondary).

**The third tab** shows the results for patients within the 3M APR-DRG v32 group for bronchiolitis.
...
Results of multivariable logistic regression models with an outcome of non-invasive ventilation are presented. Models were developed by including race/ethnicity, age, sex, insurance status, complex chronic condition flags and discharge year. Automated backward stepwise selection was performed to minimize Akaike Information Criteria. Discharge year was forced into the model as it was a variable of interest. The mixed effects models contain random intercepts for hospital region.
...
Results of multivariable logistic regression models with an outcome of icu admission are presented. Models were developed by including race/ethnicity, age, sex, insurance status, complex chronic condition flags and discharge year. Automated backward stepwise selection was performed to minimize Akaike Information Criteria. Discharge year was forced into the model as it was a variable of interest. The mixed effects models contain random intercepts for hospital region.
...
```{r icu_model_summary_primary, results="hold"}
icu_summary_primary
```

### All Bronchiolitis Diagnoses Model Characteristics

```{r icu_model_summary_all, results="hold"}
icu_summary_all
```

### APR-DRG v32 Bronchiolitis Diagnoses Model Characteristics

```{r icu_model_summary_apr_drg, results="hold"}
icu_summary_apr_drg
```
...
The **first tab** shows the proportion of admissions with bronchiolitis admitted to a single-center ICU over time. The first graph shows patients with a primary encounter diagnosis of bronchiolitis. The second graph shows all admissions to the single-center among children under two years old (with *any* diagnosis, not just bronchiolitis) for reference.

**The second tab** shows the number and percentage of patients with bronchiolitis admitted to the ICU receiving no ventilatory support (lightest gray), non-invasive ventilation (medium gray), and invasive mechanical ventilation (darkest gray).

**The third tab** shows the modified electronic PELOD-2 score among patients admitted to the single-center ICU with a primary encounter diagnosis of bronchiolitis. Patients receiving no ventilation support (high flow nasal cannula) and non-invasive ventilation were grouped together (light gray) and compared with patients receiving invasive ventilation (dark gray). 

**The fourth tab** shows the modified electronic PRISM-IV score among patients admitted to the single-center ICU with a primary encounter diagnosis of bronchiolitis. Patients receiving no ventilation support (high flow nasal cannula) and non-invasive ventilation were grouped together (light gray) and compared with patients receiving invasive ventilation (dark gray). 

Severity of illness data were available from 2010 to 2018.
...
This dashboard was built using [RStudio](https://rstudio.com/products/rstudio/) running `r sessionInfo()$R.version$version.string` on `r sessionInfo()$running`, and the following packages:

```{r colophon_table, results="hold"}
...
The total rendering time for this document was `r round(dseconds(start_time %--% end_time)/60,0)` minutes, of which `r round(dseconds(model_start_time %--% model_end_time)/60,0)` minutes were spent on model generation.
