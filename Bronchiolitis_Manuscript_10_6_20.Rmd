...
knitr::opts_chunk$set(knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE, results = "hide", scipen = 999))
library(tidyverse)
...
demographics_all_bronchiolitis <- demographics_table_function(all_bronchiolitis_diagnoses, "Sensitivity: All Bronchiolitis Diagnoses", all_bronchiolitis_diagnoses$discharge_id)
...
demographics_apr_drg_bronchiolitis <- demographics_table_function(apr_drg_bronchiolitis, "Sensitivity: APR-DRG v32 Bronchiolitis", apr_drg_bronchiolitis$discharge_id)
...
demographics_combined <- left_join(demographics_primary_bronchiolitis_diagnoses, demographics_all_bronchiolitis, by = "Variable") %>%
  left_join(., demographics_apr_drg_bronchiolitis, by = "Variable")
...
```{r number_of_hospitals, results = "markdown"}
paste("The number of hospitals in the cohort was:")

all_bronchiolitis_diagnoses %>%
...
```{r table of percent admitted to ICU by year, results = "hold"}
...
yearly_table_function <- function(bronchiolitis_table) {
  ...
  return(yearly_bronchiolitis_mortality)
}

hospital_admission_icu_table_function <- function(hospital_admission_table, graph_name) {
...
primary_bronchiolitis_icu_yearly_table <- yearly_table_function(primary_bronchiolitis_diagnoses)

hospital_icu_yearly_table <- hospital_admission_icu_table_function(all_hospital_admissions)

rm(yearly_table_function, hospital_admission_icu_table_function)

knitr::kable(primary_bronchiolitis_icu_yearly_table, caption = "Primary Bronchiolitis Diagnosis")

knitr::kable(hospital_icu_yearly_table, caption = "All Hospital Admissions Under 2 Years")
```
...
ggsave("Manuscript_Figures//Figure_1.png")
```
...
icu_ventilation_function <- function(bronchiolitis_table, graph_name) {
  ...
  temp_table %>%
    ...
    mutate(highest_vent_support = factor(highest_vent_support, levels = c("none", "noninvasive", "invasive"))) %>%
    ggplot(aes(x = factor(discharge_year), y = n, fill = highest_vent_support)) +
    geom_col(position = "stack") +
    scale_fill_manual(values = c("gray80", "gray70", "gray60"), name = "Highest Ventilator Support", labels = c("None", "Non-invasive", "Invasive")) +
...
ggsave("Manuscript_Figures//Figure_2.png")
```
...
cost_per_patient_function <- function(bronchiolitis_table, adjustment_table) {
  ...
  temp_regression <- summary(lm(inflation_adjusted_cost ~ discharge_year, data = temp_table))

  temp_table <- temp_table %>%
    group_by(discharge_year) %>%
    summarize(
      mean_cost = round(mean(inflation_adjusted_cost, na.rm = TRUE), 2)
    )
...
kable(per_patient_bea_gdp_primary_table[[1]])
```

### Per Patient Cost Regression

```{r per_patient_cost_regression,results="markdown"}
cat("Mean Cost Per Patient Regression:
    ")
...
yearly_graph_function <- function(bronchiolitis_table, inflation_table, graph_name, graph_notes) {
  ...
  yearly_plot <- ggplot(data = yearly_table, mapping = aes(x = as.factor(discharge_year), y = total_inflation_adjusted_cost, fill = factor(total_vent_support, levels = c("both", "invasive", "noninvasive", "none")))) +
    ...
    theme_bw() +
    scale_y_continuous(name = "Annual Cost (mil $)") +
    xlab("Discharge Year") +
...
cost_per_patient_function <- function(bronchiolitis_table, adjustment_table, graph_name) {
  ...
  temp_plot <- temp_table %>%
    ...
    xlab("Discharge Year") +
    scale_y_continuous(name = "Admission Cost ($)", breaks = c(0, 2500, 5000, 7500, 10000, 12500, 15000, 17500)) +
...
```{r figure_3, results="markup"}
cowplot::plot_grid(bea_gdp_annual_primary, per_patient_bea_gdp_primary, labels = "AUTO", cols = 1)
ggsave("Manuscript_Figures//Figure_3.png")
```
...
```{r evaluate_model_characteristics, results="hold"}
model_summary_function <- function(invasive_model, noninvasive_model, icu_model) {
  require(kableExtra)

  invasive_tibble <- as_tibble(summary(invasive_model)$coefficients) %>%
    mutate(Variable = rownames(summary(invasive_model)$coefficients)) %>%
    ...
    select(Variable, "Invasive Ventilation OR [95% CI]" = or_conf_int_95)

  noninvasive_tibble <- as_tibble(summary(noninvasive_model)$coefficients) %>%
    mutate(Variable = rownames(summary(noninvasive_model)$coefficients)) %>%
    mutate(odds_ratio = exp(Estimate)) %>%
    mutate(low_conf_int_95 = Estimate - `Std. Error`) %>%
    mutate(hi_conf_int_95 = Estimate + `Std. Error`) %>%
    mutate(or_low_conf_int_95 = exp(low_conf_int_95)) %>%
    mutate(or_hi_conf_int_95 = exp(hi_conf_int_95)) %>%
    mutate(or_conf_int_95 = paste(round(odds_ratio, 3), " [", round(or_low_conf_int_95, 3), ", ", round(or_hi_conf_int_95, 3), "]", sep = "")) %>%
    select(Variable, "Non-Invasive Ventilation OR [95% CI]" = or_conf_int_95)

  icu_tibble <- as_tibble(summary(icu_model)$coefficients) %>%
    mutate(Variable = rownames(summary(icu_model)$coefficients)) %>%
    mutate(odds_ratio = exp(Estimate)) %>%
    mutate(low_conf_int_95 = Estimate - `Std. Error`) %>%
    mutate(hi_conf_int_95 = Estimate + `Std. Error`) %>%
    mutate(or_low_conf_int_95 = exp(low_conf_int_95)) %>%
    mutate(or_hi_conf_int_95 = exp(hi_conf_int_95)) %>%
    mutate(or_conf_int_95 = paste(round(odds_ratio, 3), " [", round(or_low_conf_int_95, 3), ", ", round(or_hi_conf_int_95, 3), "]", sep = "")) %>%
    select(Variable, "ICU Admission OR [95% CI]" = or_conf_int_95)

  combined_tibble <- full_join(invasive_tibble, noninvasive_tibble, by = "Variable") %>%
    full_join(., icu_tibble, by = "Variable") %>%
    ...
    mutate(Variable = factor(Variable, levels = c("Discharge Year", "Sex: Female", "Race: Black", "Race: Other", "Ethnicity: Hispanic", "Ethnicity: Unknown", "Insurance: Government", "Insurance: Other", "Admit Age (months): 1st Order Spline", "Admit Age (months): 2nd Order Spline", "Admit Age (months): 3rd Order Spline", "CCC: Cardiovascular", "CCC: Congenital / Genetic Defect", "CCC: Gastrointestinal", "CCC: Hematologic / Immunologic", "CCC: Malignancy", "CCC: Metabolic", "CCC: Neurologic", "CCC: Prematurity / Neonatal", "CCC: Renal / Urologic", "CCC: Respiratory", "CCC: Technology Dependent", "CCC: TPN Dependent", "CCC: Transplant Patient"))) %>%
    arrange(Variable)
...
summary_primary_bronchiolitis_diagnoses <- model_summary_function(invasive_models_primary[[2]], noninvasive_models_primary[[2]], icu_models_primary[[2]])

rm(model_summary_function)

summary_primary_bronchiolitis_diagnoses
```
